...
if (!class_exists('Piklist_Checker'))
{
	class PiklistChecker
	{
		...
		{
			add_action('network_admin_notices', array('ChronosWP\\PiklistChecker', 'show_message'));
			add_action('admin_notices', array('ChronosWP\\PiklistChecker', 'show_message'));
		}
		...
		public static function check($this_plugin, $checking = 'plugin')
		{
			...
			if($checking == 'plugin')
			{
				...
				if (is_multisite())
				{
					...
					{
						PiklistChecker::deactivate_plugins($this_plugin, 'network');
					}
					else
					{
						PiklistChecker::deactivate_plugins($this_plugin, 'single-network');
					}
				}
				else
				{
					PiklistChecker::deactivate_plugins($this_plugin, 'single');
				}
			}
			else
			{
				PiklistChecker::$theme = true;
		...
		public static function deactivate_plugins($this_plugin, $type)
		{
			...
			foreach ($plugins as $plugin)
			{
				...
				{
					array_push(PiklistChecker::$plugins, $this_plugin);
		...
		<?php endif; ?>
			...
			<p>
				...
				<strong>
					...
					<?php foreach(PiklistChecker::$plugins as $plugin): $data = get_plugin_data($plugin); ?>

						<?php PiklistChecker::$plugin_list = PiklistChecker::$plugin_list . $data['Title'] . ', '; ?>

					<?php endforeach; ?>

					<?php echo rtrim(PiklistChecker::$plugin_list, ", "); ?>
		...
		<?php endif; ?>
			...
			<ol>
				...
				if(!empty(PiklistChecker::$plugins)) :
		...
		{
			if (!empty(PiklistChecker::$plugins) || PiklistChecker::$theme == true) : ?>

				<div class="error">

					<p>
						<?php echo PiklistChecker::message(); ?>
					</p>
